...
class RaifaisenPipeline:

    # Database setup
    conn = sqlite3.connect('raifaisen.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `raifaisen`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM raifaisen WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO raifaisen (date, title, link, content)"
